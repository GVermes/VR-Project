...
public class OVRGrabber : MonoBehaviour
{
   ...
   // public AudioClip throwBall;
    ...
    public IEnumerator TouchingSomethingTrue()
    {
        touchingSomething = true;
        yield return new WaitForSeconds(0.1f);
    }

    public IEnumerator TouchingSomethingFalse()
    {
    ...
    void OnTriggerEnter(Collider otherCollider)
    {
        ...
        StartCoroutine(TouchingSomethingTrue());
        // Add the grabbable
        int refCount = 0;
    ...
    void OnTriggerExit(Collider otherCollider)
    {
		OVRGrabbable grabbable = otherCollider.GetComponent<OVRGrabbable>() ?? otherCollider.GetComponentInParent<OVRGrabbable>();
        if (grabbable == null) return;
        StartCoroutine(TouchingSomethingFalse());
    ...
    public void GrabBeginPublic(GameObject otherCollider)
    {
        // Get the grab trigger
        OVRGrabbable grabbable = otherCollider.GetComponent<OVRGrabbable>() ?? otherCollider.GetComponentInParent<OVRGrabbable>();
        if (grabbable == null) return;
    ...
    protected void GrabEnd()
    {
        if (m_grabbedObj != null)
        {
            ...
            if (spawnBall)
            {
                RemoveFromList(m_grabbedObj);
                spawnBall = false;
            }
            //m_grabbedObj.gameObject.GetComponent<>().isNewBall = 
            GrabbableRelease(linearVelocity, angularVelocity);
           // AudioSource.PlayClipAtPoint(throwBall, Camera.main.transform.position);
        }
    ...
    void RemoveFromList(OVRGrabbable grabbable)
    {
    ...
    {
        m_grabbedObj.GrabEnd(linearVelocity * 3f, angularVelocity);
